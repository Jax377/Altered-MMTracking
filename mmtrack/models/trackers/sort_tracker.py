...
class SortTracker(BaseTracker):
    ...
    def update_track(self, id, obj, frame_id):
        """Update a track."""
        super().update_track(id, obj, frame_id)
        ...
        self.tracks[id].mean, self.tracks[id].covariance = self.kf.update(
            self.tracks[id].mean, self.tracks[id].covariance, bbox, frame_id)
    ...
    def track(self,
        ...
        if self.empty or bboxes.size(0) == 0:
            ...
            if self.with_reid:  # classification network matching
                embeds = model.reid.simple_test(
                    self.crop_imgs(reid_img, img_metas, bboxes[:, :4].clone(),
                                   rescale))
                # reid
                if len(active_ids) > 0:
                    ...
                    for r, c in zip(row, col):
                        ...
                        if dist <= self.reid['match_score_thr']:
                            ...
                            total_r.append(r)
                            total_c.append(c)
                    costs = np.delete(costs, total_r, 0)
                    costs = np.delete(costs, total_c, 1)

            active_ids = [
                id for id in self.ids if id not in ids
            ]

            if len(active_ids) > 0:  # IoU-threshold matching (0.8) [Optional for "+" matching]
                ...
                for r, c in zip(row, col):
                    dist = dists[r, c]
                    if dist < 1 - self.match_iou_thr - 0.2:
            ...
            if len(active_ids) > 0:  # IoU-threshold matching (0.965)
                active_dets = torch.nonzero(ids == -1).squeeze(1)
                track_bboxes = self.get('bboxes', active_ids)
                ious = bbox_overlaps(
                    track_bboxes, bboxes[active_dets][:, :-1]).cpu().numpy()
            ...
            if len(active_ids) > 0:  # Euclidean distance matching [Optional for "+" matching]
                active_dets = torch.nonzero(ids == -1).squeeze(1)
                track_bboxes = self.get('bboxes', active_ids)
                numpy_track_bboxes = bbox_xyxy_to_cxcyah(track_bboxes).cpu().numpy()
                numpy_det_bboxes = bbox_xyxy_to_cxcyah(bboxes[active_dets][:, :-1]).cpu().numpy()
                euclidean = np.linalg.norm(numpy_track_bboxes[:, [0, 1]][:, None, :] - numpy_det_bboxes[:, [0, 1]], axis=2)

                row, col = linear_sum_assignment(euclidean)
                for r, c in zip(row, col):
                    dist = euclidean[r, c]
                    max_dist = (numpy_track_bboxes[:, 2][0]*numpy_track_bboxes[:, 3][0]+numpy_track_bboxes[:, 3][0]/2)
                    if dist < max_dist:
                        ids[active_dets[c]] = active_ids[r]
