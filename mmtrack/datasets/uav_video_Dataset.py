...
class UAVVIDEODATASET(CocoDataset):
    ...
    def evaluate(self,
        ...
        if 'track' in metrics:
            ...
            # count = []
            # for video in track_bboxes:
            #     unique_ids = np.empty(len(video), dtype=object)
            #     for frame_nr, frame in enumerate(video):
            #         array = frame[0]
            #         unique_ids[frame_nr] = np.unique(array[:, 0])
            #     flat_array = np.concatenate(unique_ids)
            #     count_sum = len(np.unique(flat_array))
            #     count.append(count_sum)
            #
            # hand_count = [30, 33, 16, 31, 26, 50]

            bootstrapping_list = {'IDF1': np.zeros(1000),
                                  'MOTA': np.zeros(1000),
                                  'MOTP': np.zeros(1000),
                                  'FP': np.zeros(1000),
                                  'FN': np.zeros(1000),
            ...
            for k in range(1000):
                video1 = random.randint(0, 5)
                video2 = random.randint(0, 5)
                video3 = random.randint(0, 5)
                video4 = random.randint(0, 5)
                video5 = random.randint(0, 5)
                video6 = random.randint(0, 5)
                track_bboxes_shuffled = [track_bboxes[video1], track_bboxes[video2], track_bboxes[video3],
                                         track_bboxes[video4],
                                         track_bboxes[video5], track_bboxes[video6]]
                ann_infos_shuffled = [ann_infos[video1], ann_infos[video2], ann_infos[video3], ann_infos[video4],
                                      ann_infos[video5],
                                      ann_infos[video6]]

                track_eval_results = eval_mot(
                    results=track_bboxes_shuffled,
                    annotations=ann_infos_shuffled,
                    logger=logger,
                    classes=self.CLASSES,
                    **track_kwargs)
                for key in bootstrapping_list:
                    bootstrapping_list[key][k] = track_eval_results[key]

            bootstrapping_results = {'IDF1': np.zeros(6),
                                     'MOTA': np.zeros(6),
                                     'MOTP': np.zeros(6),
                                     'FP': np.zeros(6),
                                     'FN': np.zeros(6),
            ...
            for key in bootstrapping_list:
                bootstrapping_results[key][0] = np.mean(bootstrapping_list[key])
                bootstrapping_results[key][1] = np.std(bootstrapping_list[key])
                confidence_level = 0.95
                degrees_freedom = len(bootstrapping_list[key]) - 1
                confidence_interval = stats.t.interval(confidence_level, degrees_freedom, bootstrapping_results[key][0],
                                                       bootstrapping_results[key][1] / np.sqrt(
                                                           len(bootstrapping_list[key])))
                bootstrapping_results[key][2] = bootstrapping_results[key][0] - confidence_interval[0]
                bootstrapping_results[key][3] = confidence_interval[1] - bootstrapping_results[key][0]
                bootstrapping_results[key][4] = confidence_interval[0]
                bootstrapping_results[key][5] = confidence_interval[1]

            eval_results.update(bootstrapping_results)
